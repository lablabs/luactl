...
locals {
  addon_argo_source_type         = var.argo_source_type != null ? var.argo_source_type : lookup(local.addon, "argo_source_type", "helm")
  addon_argo_source_helm_enabled = local.addon_argo_source_type == "helm" || local.addon_argo_source_type == "helm-directory"

  addon_argo_name         = var.argo_name != null ? var.argo_name : lookup(local.addon, "argo_name", local.addon.name)
  addon_helm_release_name = var.helm_release_name != null ? var.helm_release_name : lookup(local.addon, "helm_release_name", local.addon.name)
  addon_helm_chart_name   = var.helm_chart_name != null ? var.helm_chart_name : lookup(local.addon, "helm_chart_name", local.addon.name)

  addon_name      = local.addon_argo_source_helm_enabled ? local.addon_helm_release_name : local.addon_argo_name
  addon_namespace = var.namespace != null ? var.namespace : lookup(local.addon, "namespace", local.addon.name)
}
